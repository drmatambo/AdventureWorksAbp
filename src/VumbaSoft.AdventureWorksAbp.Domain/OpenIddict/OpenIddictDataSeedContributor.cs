...
namespace VumbaSoft.AdventureWorksAbp.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("AdventureWorksAbp") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "AdventureWorksAbp", DisplayName = "AdventureWorksAbp API", Resources = { "AdventureWorksAbp" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["AdventureWorksAbp_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["AdventureWorksAbp_Web:RootUrl"]!.EnsureEndsWith('/');

            /* AdventureWorksAbp_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["AdventureWorksAbp_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["AdventureWorksAbp_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["AdventureWorksAbp_Swagger:RootUrl"]?.TrimEnd('/');
